...
using PeerConnectionClientCore.Utilities.time;
...
namespace PeerConnectionClient.Signalling
{
    ...
    public delegate void MessageFromPeerDelegate(object obj);
    public delegate void MessageSentDelegate(int err);
    public delegate void ServerConnectionFailureDelegate();



    public delegate void RoomNumber(object obj);
    public delegate void RoomToken(object obj);
    /// <summary>
    ...
    public class Signaller
    {
        ...
        public event MessageFromPeerDelegate OnPeerConnected;
        ...
        public event RoomNumber OnRoomNumber;
        public event RoomToken OnRoomToken;
        public event RoomToken OnPeerConnectting;
        /// <summary>
        ...
        public async void Connect(string server, string port, string client_name)
        {
            try
            {
                ...
                _targetServer = server + ":" + port;
                _state = State.SIGNING_IN;

                await ControlSocketRequestAsync();
...
#pragma warning restore 1998
        ...
        private async Task<bool> ControlSocketRequestAsync()
        {
            ...
            try
            {
                ...
                {
                    this._socket = IO.Socket(_targetServer);
                }
        ...
        {
            _socket.On(Socket.EVENT_CONNECT, OnSocketConnect);
            _socket.On("easyrtcCmd", OnSocketEasyRtcCmd);
            _socket.On(Socket.EVENT_DISCONNECT, (message) =>
            {
                Debug.WriteLine(message);
                _socket.Close();
            });
            _socket.On("authenticate", (data) =>
            {

                var msgData1 = new msgT();
                msgData1.apiVersion = "1.1.1-beta";
                msgData1.applicationName = "easyrtc.audioVideoSimple";
                msgData1.msgType = "authenticate";
                _socket.Emit("RoomJoin", JToken.FromObject(msgData1));
            });
            _socket.On("RoomJoin", (data) => { });
        ...
        private void OnEasyrtcCmd(object obj)
        {
            switch ((string)(obj as JObject).SelectToken("msgType"))
            {
                case "roomData":
                    OnRoomNumber((obj as JObject).SelectToken("msgData"));
                    break;
                case "token":
                    OnRoomToken((obj as JObject).SelectToken("msgData"));
                    break;
                default:
                    break;
            }
        ...
        private void OnEasyRtcAuth(object data)
        {
            OnEasyrtcCmd(data);
            sa = new StillAlive(_socket);
            sa.timer();
            _socket.Emit("easyrtcCmd", OnEasyrtcCmd, JToken.FromObject(new TestTT()));
        }

        public bool connectting = false;
        public bool connected = false;
        private StillAlive sa;
        ...
        private void OnSocketConnect()
        {
            ...
            msgData1.applicationName = "easyrtc.audioVideoSimple";
            msgData1.msgType = "authenticate";
            _socket.Emit("easyrtcAuth", OnEasyRtcAuth, JToken.FromObject(msgData1));
        }

        private void OnSocketEasyRtcCmd(object data)
        {
            this.OnPeerConnectting(data);
        }

        public void emit(JObject obj, Action<object> ao)
        {
            var r = JToken.FromObject(obj);
            this._socket.Emit("easyrtcCmd", ao, r);
        }
        ...
        public async Task<bool> SendToPeer(int peerId, string message)
        {
            ...
            return await ControlSocketRequestAsync();
        }
