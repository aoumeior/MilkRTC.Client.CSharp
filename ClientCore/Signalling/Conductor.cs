...
#endif
            {
                String message = evt.Candidate.Candidate;
                json = new JObject
                {
                    {"type",  JValue.CreateString("candidate") },
                    {"id", JValue.CreateString(evt.Candidate.SdpMid)},
                    { "label", index },
                    {kCandidateSdpName, JValue.CreateString(message) }
                };

                JObject or = new JObject()
                {
                    {"msgType", JValue.CreateString("candidate")  },
                    { "targetEasyrtcid", JValue.CreateString(_targetEasyrtcid)},
                    { "msgData", json},
                    {"easyrtcid", JValue.CreateString(_easyrtcid)},
                   // { "serverTime", DateTime.Now. }

                };

                    Signaller.emit(or, (data) =>
                    {
...
#endif
        public delegate void GetUserInfo(object obj);
        public event GetUserInfo OnGetUserInfo;
        /// <summary>
...
//#else
            ...
            Signaller.OnPeerConnectting += Signaller_OnMessageFromPeer;
            ...
            Signaller.OnRoomNumber += Signaller_OnRoomNumber;
            Signaller.OnRoomToken += Signaller_OnRoomToken;

            OnGetUserInfo += (a) => { };
            _iceServers = new List<RTCIceServer>();
        }

        private void Signaller_OnRoomNumber(object obj)
        {
            var o = obj as Newtonsoft.Json.Linq.JObject;
            var roomData = o.SelectToken("roomData");
        }

        public string applicationId;
        public string _easyrtcid;
        public string icestring;
        private void Signaller_OnRoomToken(object obj)
        {
            Debug.WriteLine(obj);
            var root = obj as Newtonsoft.Json.Linq.JObject;
            var roomData = root.SelectToken("roomData");
            _easyrtcid = (string)root.SelectToken("easyrtcid");
            var stillAliveInterval = root.SelectToken("stillAliveInterval");
            var applicationName = root.SelectToken("application.applicationName");

            var roomInfomation = roomData.SelectToken(applicationName.ToString());
            var roomName = roomInfomation.SelectToken("roomName");
            IEnumerable<Newtonsoft.Json.Linq.JToken> clientList = roomInfomation.SelectToken("clientList");

            icestring = root.SelectToken("iceConfig.iceServers").ToString();
            foreach (Newtonsoft.Json.Linq.JToken clinet in clientList)
            {
                OnGetUserInfo(clinet);
            }
        }
        ...
        public string _targetEasyrtcid;
        /// <summary>
        ...
        private void Signaller_OnMessageFromPeer(object messa)
        {
            var message = messa as JObject;
            _targetEasyrtcid = (string)message.SelectToken("senderEasyrtcid");
            Task.Run(async () =>
            {

                string type = (string)message.SelectToken("msgType");
                if (_peerConnection == null)
                {
                ...
                if (_peerConnection != null && type!= "candidate")
                {
                    ...
                    var sdp = (string)message.SelectToken("msgData.sdp");
                    ...
                    var des = new RTCSessionDescription(messageType, sdp);
                    await _peerConnection.SetRemoteDescription(des);
                ...
                {
                var ice = message.SelectToken("msgData");
                RTCIceCandidate candidate = new RTCIceCandidate((string)ice.SelectToken("candidate"), (string)ice.SelectToken("id"), (ushort)ice.SelectToken("label"));
        ...
        private void SendSdp(RTCSessionDescription description)
        {
            ...
            String formattedDescription = description.Sdp;



            JObject jsonf = new JObject
                    {
                        {kSessionDescriptionTypeName, JValue.CreateString("answer")},
                        {kSessionDescriptionSdpName, JValue.CreateString(formattedDescription)}
                    };

            JObject dataToShip = new JObject()
                {
                    { "msgType", JValue.CreateString("answer") },
                    { "targetEasyrtcid", JValue.CreateString(_targetEasyrtcid)},
                    { "msgData",  jsonf },
                    { "easyrtcid", _easyrtcid}
                };
            this.Signaller.emit(dataToShip, (data) => {
                Debug.WriteLine(data);
            });
        }
